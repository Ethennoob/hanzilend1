...
abstract class BaseTable {
    ...
    protected $error;
    protected $tableName;
    protected $fields; //包含所有字段的数组，key是字段名称，value是定长数组（第一个元素是数据类型（key为'type'，value为i/s/d/b），第二个元素是字段值（key为'value'，value为任何值））。
    protected $AIField; //整数类型的自动增加的字段名称
    private $sqlStmt;
    private $lastSql;
    public $conn;
    ...
    public function get($fieldsName = null, $getOne = false) {
        ...
        if ($stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                //if (count($fieldsName) === count($this->fields) && count($allRows) === 1) {//如果只有一行，且不指定查询哪些字段，则将结果赋值给当前对象。
                /* if (count($allRows) === 1) {
                  $row = $allRows[0];
